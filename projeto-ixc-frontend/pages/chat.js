...
const Chat = () => {
    ...
    const handleSendMessage = async () => {
        ...
        if (inputValue.trim() && selectedUser) {
            ...
            if (client && !client.isClosed()) {
                await client.request(selectedUser + '.msg.new', JSON.stringify(messageData));
    ...
    const handleLogoff = async () => {
        ...
        localStorage.clear();
        if (client && !client.isClosed()) {
            await client.request(userId + '.user.off', JSON.stringify(userId), { timeout: 5000 });
            await client.close();
        }
        localStorage.clear();
                    ...
                    <div className={styles.selectedUserInfo}>
                        {selectedUser ? (
                            <div className={styles.selectedUserHeader}>
                                <span
                                    className={`${styles.onlineIndicator} ${users.find((user) => user._id === selectedUser)?.isOnline
                                        ? styles.online
                                        : styles.offline
                                        }`}
                                ></span>
